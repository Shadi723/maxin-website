...
export const add_updateOuterProjects = (path: string, project: Project, files: File[] | null, history: any): void => {
    let urls: string[] = [];
    if(files !== [] ){
                ...
                async () => {
                    ...
                    if(index === files.length -1){
                        ...
                        firestore.doc(path).withConverter(projectConverter).set(project)
                            .then(res => {
                                history.push('/admin/outerprojects');
                            })
...
export const getOuterProjects= async (path: string): Promise<Project[]> => {
    let col =await firestore.collection(path).withConverter(projectConverter).get();
    return col.docs.map(project => project.data())
}

export const getOuterProject = async (path: string, id: string) : Promise<Project | undefined> => {
    let doc = await firestore.collection(path).doc(id).withConverter(projectConverter).get();
    return doc.data();
}

export const deleteOuterProject = async (path: string, id: string) : Promise<void> =>{
    let project =await getOuterProject(path, id);
    if(project !== undefined){
        project.imageUrls.forEach(async (_, index) => 
